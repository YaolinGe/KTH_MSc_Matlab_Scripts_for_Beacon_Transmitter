function [threshold_cfar, signal_cfar]= CASO_CFAR(x, T, G, offset)
...
for i = G+T+1:(Ns-(G+T))
%     
%     noise_level_left = sum(x(i:i+T-1))/T;
%     noise_level_right = sum(x(i+(T+2*G+1):i+(T+2*G)+T))/T;

    noise_level_left = sum(x(i-G-T:i-1))/T;
    noise_level_right = sum(x(i+G+1:i+(T+G)))/T;
    noise_level = min(noise_level_left, noise_level_right);
